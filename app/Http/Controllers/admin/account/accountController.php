...
class accountController extends Controller
{
    ...
    public function shop()
    {
        ...
        // total yearly income
        $yearly_income =  Orders::whereYear('created_at', Carbon::now()->year)->sum('total_profit');
        // total last year income
        $prev_year_income =  Orders::whereYear('created_at', Carbon::now()->subYear()->year)->sum('total_profit');
        //yearly percentage profit
        if ($prev_year_income != 0) {
            $yearly_increase = round(($yearly_income - $prev_year_income) / $prev_year_income * 100, 1);
        } else {
            $yearly_increase = 0;
        }
        ...
        $current_month_income = Orders::whereYear('created_at', Carbon::now()->year)
            ->whereMonth('created_at', Carbon::now()->month)->sum('total_profit');
        // total previous month income
        $last_month_income = Orders::whereYear('created_at', Carbon::now()->year)
            ->whereMonth('created_at', Carbon::now()->subMonth()->month)->sum('total_profit');
        // monthly percentage profit
        if ($last_month_income != 0) {
            $monthly_percentage_increase = round(($current_month_income - $last_month_income) / $last_month_income * 100, 1);
        } else {
            $monthly_percentage_increase = 0;
        }
        ...
        // weekly income
        $weekly_income = Orders::whereMonth('created_at', Carbon::now()->month)
        ->whereBetween('created_at', [Carbon::now()->startOfWeek(), Carbon::now()->endOfWeek()])->sum('total_profit');
        // last week income
        $last_week_income = Orders::whereMonth('created_at', Carbon::now()->month)
        ->whereBetween('created_at', [Carbon::now()->subWeek()->startOfWeek(), Carbon::now()->subWeek()->endOfWeek()])->sum('total_profit');
        ...
        $total_income = 0;
        $total_tailor_fee = 0;
        $fashion_bookings = fashionBooking::whereMonth('created_at', Carbon::now()->month)->whereBetween('created_at', [Carbon::now()->subWeek()->startOfWeek(), Carbon::now()->subWeek()->endOfWeek()])->get();
        if ($fashion_bookings) {
        ...
        // last week Customers
        $last_week_users = User::where('role', 'User')
        ...
        $last_week_bookings = fashionBooking::whereMonth('created_at', Carbon::now()->month)
        ->whereBetween('created_at', [Carbon::now()->subWeek(), Carbon::now()])->count();
        ...
        return view(
            'admin.pages.account.index',
            compact(
                'yearly_income',
                'yearly_increase',
                'current_month_income',
                'monthly_percentage_increase',
                'daily_income',
